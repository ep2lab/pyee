...
# 2) Then start interactive container 'docker run -ti --rm --name pyee_cont dolfinx/dolfinx' (or the name of the image) 
...
# 6) Now save the running container as a new image for example pyee/dolfinx:local 
        # docker commit pyee_cont pyee/dolfinx:local

# Now to RUN pyee:
# default mode:
# docker run --rm -v $PYEE:/root -u $(id -u):$(id -g) --name pyee pyee/dolfinx:local /bin/bash pyee.sh example
# Or in hyphen mode:
# docker run --rm -v $PYEE:/root -u $(id -u):$(id -g) --name pyee pyee/dolfinx:local /bin/bash pyee.sh HIP01_w hyphen
# Or solve  pre-processed simulation:
# docker run --rm -v $PYEE:/root -u $(id -u):$(id -g) --name pyee pyee/dolfinx:local /bin/bash pyee.sh HIP01_w solve_pre
...
docker run --rm -v $PYEE:/root -u $(id -u):$(id -g) --name pyee pyee/dolfinx:local /bin/bash pyee.sh $1 $2